...
class CreateObjectStorageSubUserCommand(AbstractPrintableTable, AbstractReadInputValue, AbstractPrintableJSON):
    _printable_attributes = EnumObjectStoragePrintableAttributes
    _table_headers = ["UserSrn", "DisplayName", "ClientId", "Access Key", "Secret Key"]

    def __init__(self, client_id: str, display_name: str, access: str,  configuration: vvcli_sdk.Configuration):
    ...
    async def _gen_table_rows(self, obj_users: List[dict]):
        ...
        for obj_u in obj_users:
            key = obj_u["keys"].pop()
            obj_users_info.append((obj_u["userSrn"], obj_u["displayName"], obj_u["clientId"], key["accessKey"], key["secretKey"]))
    ...
    async def _validate(self):
        ...
        self._access = await self._read_prompt_input(
            self._printable_attributes.ACCESS.value, self._access, type=str
        )
    ...
    async def execute(self, return_json=False):
        ...
        try:
            with vvcli_sdk.ApiClient(self._configuration) as api_client:
                ...
                new_subuser = vvcli_sdk.CreateSubUserObjStorageSchema(client_id=self._client_id, display_name=self._display_name, access=self._access)
