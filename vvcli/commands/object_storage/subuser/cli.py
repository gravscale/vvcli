...
@click.option(
    ...
    help="Access permissions: read, write, readwrite",
)
@click.option(
    "--json",
    "-j",
...
async def add_subuser(obj, client_id: str, display_name: str, access: str, json: bool):
    ...
    await CreateObjectStorageSubUserCommand(
        client_id, display_name, access, cli_config.load_sdk_configuration()
    ).execute(json)
...
async def list_subuser(obj, client_id: str, json: bool):
    cli_config: CliConfiguration = obj["config"]
    await GetSubUserObjectStorageCommand(
